...
{
    auto& loadMatrix = projectContext.loadMatrix;
    ...
    for (std::size_t i = 0; i < nodes.size(); i++) {
        for (std::size_t j = i + 1; j < nodes.size(); j++) {
            if (loadMatrix(i, j) > 0 && loadMatrix(j, i) > 0) {
                ...
                channel.capacity = loadMatrix(i, j);
...
std::tuple<Distances, VertexDescriptors> NetworkGraph::dijkstra(std::uint32_t src, std::uint32_t Channel::* weight) noexcept
{
